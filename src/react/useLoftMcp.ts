...
export const LoftMcpContext = createContext<LoftMcpValue | null>(null)

export function useLoftMcp(): LoftMcpValue {
  const value = useContext(LoftMcpContext)
  if (!value) throw new Error('useLoftMcp must be used inside a <LoftMcpProvider>')
  return value
}
