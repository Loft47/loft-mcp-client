...
export interface LoftMcpClient {
  ...
  revoke(opts?: { purge?: boolean }): void
...
export function createLoftMcpClient(options: LoftMcpClientOptions): LoftMcpClient {
  ...
  const environmentOrNull = (): LoftEnvironment | null => {
    const id = keys.environmentId
    return id ? (registry[id] ?? null) : null
  }

  const requireEnvironment = (): LoftEnvironment => {
    const env = environmentOrNull()
    if (!env) throw new NoEnvironmentError()
    return env
  }

  const baseUrl = options.baseUrl ?? ((env: LoftEnvironment) => env.proxyPrefix)
  ...
  if (environmentOrNull()) transport.primeSession()
  ...
  return {
    ...
    setEnvironment(id) {
      if (!registry[id]) throw new Error(`Unknown environment: ${id}`)
    ...
    brokerageKey(name, brokerageId) {
      ...
      if (id === null) throw new Error('No brokerage selected')
